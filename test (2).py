...
def load_from_github(urls, min_f, threshold, _version="v5.17"):
    def fetch(url):
...
_anon_col1, _anon_col2 = st.columns([8, 2])
with _anon_col2:
...
with tab6:
    ...
    @st.cache_data(show_spinner=False)
    def load_indicators(url, _version="v5.17"):
        if not url or url.startswith("REPLACE"):
            return None
        try:
    ...
    ind_df = load_indicators(GITHUB_URLS.get("indicators", ""), _version="v5.17")
...
if ANONYMIZE and "_AnonCode" in all_phys.columns:
    ...
    if "FullName" in all_phys.columns:
        all_phys["FullName"]   = all_phys["_AnonCode"]
else:
    all_phys["Physician Name"] = all_phys["FullName"].fillna(
...
with tab7:
    ...
    _ind_for_ctx = load_indicators(GITHUB_URLS.get("indicators", ""), _version="v5.17") if "load_indicators" in dir() else None
